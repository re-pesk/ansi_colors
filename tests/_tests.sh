...
test_runExtColorTests() {
  ...
  result="$(test_extColors "Extended Colors 0-15" "$EF_EXT" 0 15 "$(
    cat <<EOF
...
EOF
  ...
  for i in $(seq 0 5); do
    ...
    result="$(test_extColors "Extended Colors $((i * 36 + 16))-$((i * 36 + 51))" "$EF_EXT" $((i * 36 + 16)) $((i * 36 + 51)) "$(test_getLine "$expected" "$i")")"
    echo "$(test_firstLine "$result")" >&2
  ...
  result="$(test_extColors "Extended Colors 232-255" "$EF_EXT" 232 255 "$(
    cat <<EOF
...
EOF
  ...
  result="$(test_extColors "Extended Colors (background) 0-15" "$EB_EXT" 0 15 "$(
    cat <<EOF
...
EOF
  ...
  for i in $(seq 0 5); do
    # test_printExtColors "$EF_EXT" $(seq $((i * 36 + 16)) $((i * 36 + 51))) # | cat -vte >&2
    result="$(test_extColors "Extended Colors (background) $((i * 36 + 16))-$((i * 36 + 51))" "$EB_EXT" $((i * 36 + 16)) $((i * 36 + 51)) "$(test_getLine "$expected" "$i")")"
    echo "$(test_firstLine "$result")" >&2
    messages="${messages}${nl}$(test_getLine "$result" 1)"
    echo "$(test_lastLine "$result")"
  done
  ...
  # test_printExtColors "${EF_EXT}" $(seq 232 255) | cat -vte >&2
  result="$(test_extColors "Extended Colors (background) 232-255" "$EB_EXT" 232 255 "$(
    cat <<EOF
...
test_report test_runExtColorTests
